...
class Simulator:
    ...
    def recommend_next(self, user_answer=None, quiz=False):
        ...
        # 5번째 문제부터는 사용자의 학습 수준에 맞게 추천
            ...
            item_index = self.selector.select(items=self.result_array, administered_items=administered_items, est_theta=est_theta)
...
#     # 다음 추천 문제 및 사용자의 학습 수준 반환
#     item_id = s.recommend_next(quiz=True)
...
#     est_theta, administered_items, responses, index = s.recommend_next(answer.answer)
...
async def submit_answer(answer: Answer, response: Response):
    ...
    # 서버로부터 응답을 전송하는 로직
    est_theta, administered_items, responses, index, last_quiz = s.recommend_next(answer.answer)
